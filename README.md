...
```text
local-bible-reader-offline-1.10.0-release.apk
```
...
```text
D:\bible-reader\dist\android\local-bible-reader-offline-1.10.0-release.apk
```
...
- 经文右键 / 长按菜单、多选复制
- 左右滑翻章、键盘左右键
- 按住口令：小米 ASR 听写后，用同一把 Key 调用 mimo-v2.5 理解并跳转或搜索
- 导入 / 导出个人数据
- 本地诊断
- Electron 桌面入口
