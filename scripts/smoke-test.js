...
assert(staticJs.includes("Bible.dom"), "Bible.dom missing");
assert(staticJs.includes("Bible.bus"), "Bible.bus missing");
assert(staticJs.includes("function showStatus"), "showStatus must stay a direct function");
...
assert(mapData.places.some((item) => item.id === "sinai"), "Sinai missing from map gazetteer");
assert(indexHtml.includes("id=\"verseMenuMapBtn\""), "Verse map menu missing");
assert(indexHtml.includes("id=\"mapVerseBanner\""), "Verse map banner missing");
assert(staticJs.includes("openMapFromVerse"), "Verse map opener missing");
