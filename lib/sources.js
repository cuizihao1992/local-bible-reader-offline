...
import { looksEncrypted, looksReadable } from "./text.js";
        ...
        try {
          ...
          try {
            if (hasTable(db, "commentary")) {
              ...
              if (String(metadata.encrypt || "") === "1") readable = false;
              else {
                const sample = db.prepare("select Data from commentary where Data is not null and Data <> '' limit 1").get();
                readable = !looksEncrypted(sample?.Data || "") && looksReadable(sample?.Data || "");
              }
            }
          } finally {
            db.close();
      ...
      .map((entry) => {
        ...
        try {
          ...
          return {
            ...
            count,
            imageCount,
            readable: String(metadata.encrypt || "") !== "1" && !looksEncrypted(sample?.Description || "") && looksReadable(sample?.Description || ""),
          };
        } finally {
          db.close();
        }
