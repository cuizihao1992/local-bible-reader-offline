...
import { cleanText, decodeModuleText, extractStrongNumbers, parseImageNames } from "./text.js";
...
export function createReader(sources) {
  ...
  function likePattern(keyword, fuzzy) {
    const cleaned = String(keyword || "").replace(/\s+/g, "");
    const escape = (value) => String(value).replace(/\\/g, "\\\\").replace(/%/g, "\\%").replace(/_/g, "\\_");
    if (fuzzy && [...cleaned].length >= 2) return `%${[...cleaned].map(escape).join("%")}%`;
    return `%${escape(cleaned || keyword)}%`;
  }
  ...
  function searchBible(versionId, query, options = {}) {
    ...
    const fuzzy = options.fuzzy === true || options.fuzzy === "1" || options.fuzzy === "true";
    ...
    const params = [likePattern(keyword, fuzzy)];
    const where = ["Scripture like ? escape '\\'"];
    ...
    const rankBook = currentBook > 0 ? currentBook : 0;
    const db = new DatabaseSync(sources.biblePath(versionId), { readOnly: true });
    try {
      const rows = db
        .prepare(
          `select Book, Chapter, Verse, Scripture
           ...
           order by case when Book = ? then 0 else 1 end, Book, Chapter, Verse
           limit ? offset ?`,
        )
        .all(...params, rankBook, limit + 1, offset);
      const books = sources.getBooks(versionId);
  ...
  function moduleImageUrl(kind, sourceId, name) {
    const pathName = kind === "commentary" ? "/api/commentary/image" : "/api/dictionary/image";
    return `${pathName}?source=${encodeURIComponent(sourceId)}&name=${encodeURIComponent(name)}`;
  }

  function entryImages(kind, sourceId, names) {
    return names.map((name) => ({
      name,
      url: moduleImageUrl(kind, sourceId, name),
    }));
  }
  ...
  function getCommentary(sourceId, book, chapter) {
    ...
    try {
      ...
      const hasImagesCol = sources.hasColumn(db, "commentary", "Images");
      const rows = db
        .prepare(
          hasImagesCol
            ? `select Book, Chapter, FromVerse, ToVerse, Data, Images
               from commentary
      ...
      const entries = rows.map((row) => {
        const decoded = decodeModuleText(row.Data);
        const names = [...decoded.images, ...parseImageNames(row.Images || "")];
        ...
        return {
          ...
          images: entryImages("commentary", sourceId, unique),
          hasImages: unique.length > 0,
        };
      });
      const readable = entries.some((entry) => entry.text) || !entries.some((entry) => entry.encrypted);
      return {
        source: sourceId,
    ...
    try {
      ...
      return {
        ...
        results: rows.map((row) => {
          const decoded = decodeModuleText(row.Description || "");
          const names = [...decoded.images, ...parseImageNames(row.Images || "")];
          const unique = [...new Set(names)];
          return {
            id: Number(row.id),
            ...
            text: decoded.text,
            encrypted: decoded.encrypted,
            images: entryImages("dictionary", sourceId, unique),
          };
        }),
      };
    } finally {
  ...
  function sendModuleImage(res, filePath, imageName) {
    const db = new DatabaseSync(filePath, { readOnly: true });
    try {
      if (!sources.hasTable(db, "Images")) throw httpError("这个资料库没有图片表", 404);
      const row = db.prepare("select FileName, Data from Images where FileName=?").get(imageName);
      if (!row) throw httpError("找不到图片", 404);
      const ext = path.extname(String(row.FileName || imageName)).toLowerCase();
  ...
  function sendDictionaryImage(res, sourceId, imageName) {
    sendModuleImage(res, sources.dictionaryPath(sourceId), imageName);
  }

  function sendCommentaryImage(res, sourceId, imageName) {
    sendModuleImage(res, sources.commentaryPath(sourceId), imageName);
  }
