...
export function createUserStore({ userDbPath, userDataDir, biblePath }) {
  ...
  function getAllMarks(filter = {}) {
    ...
    try {
      ...
      else if (filter.kind === "note") where.push("(note <> '' or tags <> '')");
      else if (filter.kind === "highlight" || filter.kind === "highlighted") {
        where.push("(highlighted = 1 or coalesce(highlight_color, '') <> '')");
      } else {
        where.push("(favorite = 1 or highlighted = 1 or coalesce(highlight_color, '') <> '' or note <> '' or tags <> '')");
      }
      if (filter.tag) {
